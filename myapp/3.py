...
class DashboardView(ListView):
    ...
    def get_queryset(self):
        ...
        if user.is_superuser:
            ...
            queryset = Vehicle.objects.filter(seller=user).order_by('-created_at')
    ...
    def get_context_data(self, **kwargs):
        ...
        if user.is_superuser:
            ...
            all_vehicles_count = Vehicle.objects.filter(seller=user).count()
            sold_count = Vehicle.objects.filter(seller=user, is_available=False).count()
            unsold_count = Vehicle.objects.filter(seller=user, is_available=True).count()
